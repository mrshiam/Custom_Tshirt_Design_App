...
//custom js-------------------------------------------------------------->
  ...
  //change product----------------
  var canvasWrapper = document.getElementById('tshirt_image');

    function changeBackgroundImage(imageUrl) {
      canvasWrapper.style.backgroundImage = 'url(' + imageUrl + ')';
    }
  ...
  var imageList = [];
    ...
    function handleImageUpload() {
      ...
      if (fileInput.files && fileInput.files[0]) {
        ...
        // When the image is loaded, add it to the image list
        reader.onload = function(e) {
          var image = new Image();
          image.onload = function() {
            var fabricImage = new fabric.Image(image);

            // Resize the fabric image if needed
    ...
    function updateImageList() {
      ...
      // Create image list items for each image in the list
      for (var i = 0; i < imageList.length; i++) {
        var imageListItem = document.createElement('div');
        imageListItem.className = 'image-list-item';
        imageListItem.setAttribute('data-index', i);
        imageListItem.onclick = selectImageFromList;
        ...
        icon.onclick = removeImageFromList;

        // Append the image and icon to the image list item
        imageListItem.appendChild(image);
        imageListItem.appendChild(icon);

        // Add the image list item to the container
    ...
    function selectImageFromList() {
      ...
      if (index === selectedImageIndex) {
        deselectImageFromList();
      } else {
        selectedImageIndex = index;

        var imageListItems = document.getElementsByClassName('image-list-item');
        for (var i = 0; i < imageListItems.length; i++) {
          imageListItems[i].classList.remove('selected');
        }
        ...
        selectedImage.onload = function() {
          var fabricImage = new fabric.Image(selectedImage);
          canvas.add(fabricImage);
        };
        selectedImage.src = imageList[selectedImageIndex];
      }
    }

    function deselectImageFromList() {
    ...
    function removeImageFromList(event) {
  ...
  const backgroundImageUrl = getComputedStyle(tshirtImage).getPropertyValue("background-image").replace(/^url\(['"](.+)['"]\)/, '$1');

  // Use html2canvas to capture the tshirt_image div as an image
  html2canvas(tshirtImage).then(function(capturedCanvas) {
    // Get the captured image data
    const capturedImageData = capturedCanvas.toDataURL("image/png");

    // Create an image element for displaying the saved image in the list
    const image = document.createElement("img");
    image.src = capturedImageData;

    // Create a list item element and append the image to it
    // const listItem = document.createElement("li");
    // listItem.appendChild(image);

    // Append the list item to the saved_image div
    const savedImageDiv = document.querySelector(".saved_image");
    savedImageDiv.appendChild(image);
  });
});
...
//Download
  ...
  html2canvas(tshirtImage).then(function(capturedCanvas) {
    ...
    link.download = "custom_product_" + randomNumber + ".png";
...
$(document).ready(function() {
  $('.custom_design_btn').on('click', function() {
    $('#bd-overlay').fadeIn();
    $('#bd-overlay').css({
      'display': 'flex'
    });
...
$(document).ready(function() {
  $("#nextButton").on("click", function() {
    $(".canvas-div").addClass("swipe-left");
    setTimeout(function() {
      $(".canvas-div").hide().removeClass("swipe-left");
      $(".cart-div").show().addClass("swipe-right");
      $("#nextButton").hide();
      $("#previousButton").show();
      $(".cart-div").removeClass("swipe-right");
    }, 500);
  });

  $("#previousButton").on("click", function() {
    $(".cart-div").addClass("swipe-left");
    setTimeout(function() {
      $(".cart-div").hide().removeClass("swipe-left");
      $(".canvas-div").show().addClass("swipe-right");
      $("#previousButton").hide();
      $("#nextButton").show();
      $(".canvas-div").removeClass("swipe-right");
    }, 500);
  });
  $("#closeButton").on("click", function() {
    $("#bd-overlay").css("display", "none");
  });

   $('.list-tooltip').click(function(event) {
  // Remove active class from all list items
  $('.list-tooltip').removeClass('active');
...
//end custom js------------------------------------------------------------------->
  ...
  function addAccessors($scope) {
    ...
    $scope.addDesign = function(imagePath) {
    ...
    function addTexts() {
      var iText = new fabric.IText('Default Text', {
        fontSize: 20,
        left:50,
        top: 0,
        fontFamily: 'Helvetica',
  ...
  customTshirtDesign.controller('CanvasControls', function($scope) {
